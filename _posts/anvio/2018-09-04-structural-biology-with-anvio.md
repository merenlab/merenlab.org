...
The itinerary for this post is to first discuss how to set up your computational environment. Afterwards, it will discuss the structure database is in-depth. Finally it will explain how to interactively visualize environmental variants on protein structures.
...
{:.warning}
We, the developers of anvi'o, are not structural biologists and we claim no authority on reference-based protein homology modelling. We are just taking MODELLER, which is a long-standing gold standard in protein prediction, and making it accessible to microbial ecologists and their data. If you disagree with what were saying, we're probably wrong and would appreciate you correcting us.
...
These two proteins are [3DFR](https://www.rcsb.org/structure/3dfr) and [4DFR](https://www.rcsb.org/structure/4dfr) and you may be surprised to know that they only share 29% sequence identity, with a root-mean square difference (RMSD) between their main chain atoms of 3.927 [ångström](https://en.wikipedia.org/wiki/%C3%85ngstr%C3%B6m). Visually, you can see that it is not much difference. In the paper from which I found these proteins, they looked at a total of 34 pairs of homologous proteins (which was totally badass in 1986) and they found the following relationship between sequence similarity and RMSD:
...
Based on the results of [this MODELLER paper](https://www.ncbi.nlm.nih.gov/pubmed/18428767), as well as several protein experts I've talked to, 30% sequence similarity with few extended gap regions is around the lower limit of when structures can be modelled through homology and the results can be trusted within a couple ångströms. Obviously, drawing parallels between characteristics such as binding pocket geometry should be taken with a grain of salt at this level of similarity, as functions can differ drastically.
...
{:.warning}
If you are a structural biologist guru and there's something you want to control but that we don't provide access to, please let us know and we'll try our best to add it.
...
The remainder of this tutorial describes the two ways in which you can provide anvi'o with sequence variability information, which is required to run the interface. The easiest and most straightforward way is to *not* provide anything, and let anvi'o do it for you. Simply provide the corresponding contigs and profile databases alongside your structure database:
...
In this case anvi'o will calculate both SCVs and SAAVs on-the-fly and the following messages will appear in your terminal:
...
The first command has some bells and whistles that are worth describing. First, `-s /PATH/TO/STRUCTURE.DB` is provided so that the structural information is added into the variability outuput. Second, the flag `--engine AA` tells anvi'o to report SAAVs (for SCVs or SNVs you cold use `CDN` or `NT`, respectively). Thirdly, `--only-if-structure` subsets your genes of interest to only include those with a structure in the structure database. That means if you have 1000 genes in your contigs database but only 3 have structure, variability will only be generated for those 3. Finally, `-o variability.txt` is the name of the output file.
...
We hope that this tutorial has been useful for you. If you have any suggestions or questions please contact us. If you discover any bugs, please create an issue on [anvi'o GitHub repository](https://github.com/merenlab/anvio).
