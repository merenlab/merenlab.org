...
{:.notice}
<b>IMPORTANT NOTE for MAC users</b>: When you export a TAB-delimited text file from EXCEL, it will not work properly with the pipeline, since EXCEL fails to insert proper "newline" characters into these files. There are many ways to fix the file, but if you are not familiar with this issue, I would suggest you to download this <a href="https://sourceforge.net/projects/dos2unix/">little command line tool</a> to correct EXCEL's failure to perform one of the simplest tasks imaginable.
