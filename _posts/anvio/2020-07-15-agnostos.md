...
Import agnostos functions to the external E. faecalis genomes
```r
...
Import agnostos categories as (functions) to each contigs DB for the external genomes
```bash
...
Now let's separate the E. faecalis bin from the IGD metagenome and import Agnostos categories to it

Enter Gene-mode for E. faecalis
```bash
anvi-script-add-default-collection -p INFANT-GUT-TUTORIAL/PROFILE.db
...
Import functions into internal E. faecalis genome
```r
# E. faecalis gene-caller-ids from merged profile db
e_faecalis_bin_gene_level_coverage_stats <- read_tsv("INFANT-GUT-TUTORIAL/GENES/default-E_facealis.tsv")

# Join table to get E. faecalis specific cluster info
e_faecalis_unk_categories <- e_faecalis_bin_gene_level_coverage_stats %>% 
                                                left_join(IGD_genes_summary_info_exp) %>%
                                                select(gene_callers_id, category) %>% 
                                                distinct()

# Angostos categories
IGD_genes_summary_info_exp <- read_tsv("IGD_genes_summary_info_exp_new.tsv")

# Import functions for entire metagenome
agnostos_functions_for_metagenome <- IGD_genes_summary_info_exp %>%
  mutate(source = "Agnostos_categories") %>%
  rename("accession" = cl_name, "function" = category) %>%
  mutate(e_value = 0) %>%
  select(gene_callers_id, source, accession, "function", e_value)
...
# export table
write_tsv(agnostos_functions_for_metagenome, path = "Agnostos_functions_for_metagenome.tsv", col_names = TRUE)
```
