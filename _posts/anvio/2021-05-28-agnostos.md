...
{:.notice}
This section is meant to compare gene cluster methods between AGNOSTOS and anvi'o. If you are interested in exploring genes of unknown function please skip to the next section [Exploring genes of unknown functions](#exploring-genes-of-unknown-functions).

FIXME: zip this director `~/scratch/INFANT-GUT-TUTORIAL-now/ebame6/data` and make a figshare link

Just as a reminder, the anvi'o pangenome calculated GCs to learn about the extent of gene content shared between Enterococcus genomes, but remember, AGNOSTOS has it's own gene clusters (we know that's confusing, we'll explain in a second). AGNOSTOS has two layers in the pangenome you should consider `AGNOSTOS number of clusters` and `Agnostos categories`. The first layer contains the number of AGNOSTOS GCs contained in a anvi'o GC. The second layer includes the proportions of AGNOSTOS GC categories in an anvi'o GC (what... a GC can have multiple categories... yes C'est la vie, we will come back to this later).
...
Check out this pattern (be sure to click "Highlight splits on the tree" to mark target GCs after searching). The GCs with low [homogeneity](https://merenlab.org/2016/11/08/pangenomics-v2/#inferring-the-homogeneity-of-gene-clusters) tend to contain more AGNOSTOS GCs.
...
Another way to explore differences between anvi'o and AGNOSTOS gene clustering methods is to order by the AGNOSTOS categories in the interactive interface:
...
We will focus on the group of GCs that are part of the Genomic Unknowns (GUs) and also classified by anvi'o as SCGs. These sets of genes have a lot of potential reveal novel biology because they are conserved in the pangenome and are yet to be classified!

{% include IMAGE path="/images/agnostos/agnostos-highlight-scgs.png" caption="Figure asdf" %}

{% include IMAGE path="/images/agnostos/agnostos-pan-scgs.png" caption="Figure asdf" %}
...
# external-genomes AGNOSTOS output: `~/scratch/INFANT-GUT-TUTORIAL-now/ebame6/data/IGD_ext_genomes_summary_info_exp.tsv`
# anvi'o pangenome enterococcus summar file: `Enterococcus_gene_clusters_summary.txt.gz`
# Join those two files to create the file below
agnostos_pan_data <- read_tsv(file = "data/agnostos-pan-data.tsv.gz")

# Subset of Vanni et al., 2022 GTDB integrated data
agnostos_gtdb <- read_tsv(file = "data/agnostos-gtdb.tsv.gz")
```

Now let's explore those Single-copy core genes that are Genomic Unknowns by leveraging the metadata that comes with AGNOSTOS. One question we can ask is do the SCG GUs occur broadly or narrowly across the tree of life? To figure this out we need calculate how many times a GU GC can be found in a specific taxonomic rank and how many times it occurs in a specific taxa. If the difference between these counts is 0, that means that the GU GC is highly specifc to a taxa. On the other hand, if the difference is large, that means that a GU GC is found across multiple taxa and thus is broadly distributed.


First lets subset the ORFs from the pangenome to just the GU SCGs:

```r
# Filter for GUs that are SCGs
gu_sgc <- agnostos_pan_data %>%
  filter(category == "GU", SCG==1)
```

Next let's calculate the number of occurrences GU GCs are found in taxonomic ranks in GTDB:
```r
...
Now let's calculate the number of times GU GCs occur in specific taxa and join it with the previous dataframe to learn about the phylogenetic breadth of SCG GUs from our pangenome:
...
|--------|-----------------|--|------|-----|----|
| 21178033|Lactobacillaceae  |  5|      5|genus |    0|
| 25250206|Enterococcaceae   |  1|      1|genus |    0|
| 33928417|Aerococcaceae     |  1|      1|genus |    0|
| 34710295|Flavobacteriaceae |  2|      2|genus |    0|
| 35750075|Enterococcaceae   |  2|      2|genus |    0|
| 36056373|Lactobacillaceae  | 12|     12|genus |    0|
| 36109896|Enterococcaceae   |  4|      4|genus |    0|
| 36488095|Enterococcaceae   |  9|      9|genus |    0|
| 36488670|Enterococcaceae   | 11|     11|genus |    0|
| 36507161|Enterococcaceae   | 10|     10|genus |    0|
| 37931631|Lactobacillaceae  |  5|      5|genus |    0|
| 37936696|Lactobacillaceae  |  1|      1|genus |    0|
| 37937540|Lactobacillaceae  |  6|      6|genus |    0|
| 38066942|Aerococcaceae     |  1|      1|genus |    0|
| 38238432|Streptococcaceae  |  1|      1|genus |    0|
| 38533639|Enterococcaceae   |  1|      1|genus |    0|
| 38533689|Enterococcaceae   |  2|      2|genus |    0|
| 38534067|Enterococcaceae   |  9|      9|genus |    0|
| 38819638|Enterococcaceae   |  6|      6|genus |    0|
| 38820976|Enterococcaceae   |  2|      2|genus |    0|


And which a broader distribution?
...
FIXME: add conclusion to this section
