...
To put the nail in the coffin RE (???) clustering differences between anvi'o and AGNOSTOS, let's explore the AGNOSTOS category Knowns:
...
(Chiara) I think this is not divieded from the above analysis. First we check the general distribution of the Genomic unknown GCs, or the proportion of how many are broadly and narrow distributed. Then, we check specifically which GCs have a broader or narrower distribution.
