...
This is by no means an exhaustive comparison of these mapping software, and our results are only meaningful within the narrow parameter space we explored. To avoid a biased comparison, parameters were mostly author-suggested defaults.
...
Probably by optimizing parameters of BBMAP, SMALT, GSNAP, and Novoalign, it could have been possible to get them to a level where they produce comparable results to the other four to a better extent. Regardless, these results show that one should be very careful with parameters of mapping when mapping metagenomic short reads to contigs.

Unsupervised genome binning approaches that pay attention to coverage (i.e., CONCOCT, ESOMs, or GroupM), as well as the supervised ones will definitely benefit from accurate coverage estimates for contigs. I think mapping is one step people never go back and check carefully, but clearly it is quite necessary to do so. I saw some irregular mapping results with Bowtie2 occasionally while inspecting splits. But Bowtie, BWA and CLC overall did a magnificent job with this small -yet somewhat challenging- dataset.

Anvi'o did a good job compared to other ways of comparing mapping software as it allowed us to determine which mapping results made more biological sense. As Bing put very eloquently during one of our discussions, "the portion of reads that can be mapped is one factor, but not necessarily the most appropriate", as it is sometimes even misleading.
