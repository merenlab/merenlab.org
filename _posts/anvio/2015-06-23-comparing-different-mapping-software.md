...
However, comparing the efficiency of mapping software is not an easy task if you want to do it with "real world" data. Using a very simple metagenome, we profiled eight mapping software, and here you will find a brief overview of our findings.

This is by no means an exhaustive comparison of these mapping software, and our results are only meaningful within the narrow parameter space we explored for them --which literally consisted of the "default" or "suggested" parameters for each.

But beyond our preliminary findings on the performance of multiple mappers, this little project gives a good idea about how versatile [anvi'o]({{site_url}}/projects/anvio/) is as a platform, and its potential as a benchmaraking environment.
...
In less than a minute, two near-complete bacterial genomes. Not bad!
...
The Y-axes show the coverage values, and the X-axis shows the nucleotide position on the split. Because it was quite crazy for BBMAP, I removed SNP scores that are shown for each nucleotide position (they were so many of them that it was not possible to see the coverage of short reads). In this case CLC, BWA, Bowtie, and Bowtie2 is pretty much identical. In fact the coverage of short reads they mapped to this particular split is so identical that it gives a sense of trust that they definitely did a good job. On the other hand BBMAP, SMALT, GSNAP, and Novoalign mapped too many reads to this split they should have, especially to the last 100-150nt part. Interestingly, BBMAP just went crazy, and filled the entirety of the split with reads with a steady increase in coverage (see the X-axis). Although in this particular case all of them map vast number of reads at the end of the contig, inspecting other splits gave me the impression that SMALT, GSNAP, and Novoalign always make similar mistakes (either they all over-map to the beginning, or to the end, or both ends of a split), and BBMAP most of the time does its own thing (and most of the time maps everything everywhere).

# Final words

Probably by optimizing the parameters of BBMAP, SMALT, GSNAP, and Novoalign it could have been possible to get them to a level where they produce comparable results to the other four. Regardless, these results show that one should be very careful with parameters of mapping when mapping metagenomic short reads to contigs. Unsupervised genome binning approaches that pay attention to coverage (i.e., CONCOCT, ESOMs, or GroupM), as well as the supervised ones will definitely benefit from accurate coverage estimates for contigs. I think mapping is one step people never go back and check carefully, but clearly it is quite necessary to do so.

I saw some irregular mapping results with Bowtie2 occasionally while inspecting splits. But Bowtie, BWA and CLC overall did a magnificent job with this small -yet somewhat challenging- dataset.
