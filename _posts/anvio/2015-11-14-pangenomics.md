...
We clustered transleted protein sequences for 47,415 open reading frames in these genomes using [ITEP](https://price.systemsbiology.org/research/itep/) ("Integrated Toolkit for Exploration of Pan-genomes", a useful tool we were introduced by our colleague [Rika Anderson](https://twitter.com/RikaEAnderson?lang=fr)). ITEP identified a total of 7,795 gene clusters in our collection, where each cluster contained 1 to 102 proteins. 

The main purpose of this step is to create a basic matrix to connect genomes protein clusters. Although we used ITEP, any other solution to cluster protein sequences could have been used to acquire this essential input file.

We stored this information as a matrix of protein clusters and their occurrences across 10 genomes (see `data.txt` in our figshare archive) which should looks like this, where each cell shows the count of a given protein cluster (first column) in each genome (first row):
...
Note the `--transopse` flag. The resulting tree will describe the clustering of genomes with respect to protein clusters they harbor. We stored the resulting tree in `samples-order.txt` file. We also have another file that provides us with more information about our genomes. This file is called `samples-information.txt`, and it looks like this:
...
</div>As expected, genomic clustering is consistent with taxonomical affiliations. EC and Se genomes share a large portion of protein clusters. On the other hand, only a small number of protein clusters are common to all 30 genomes. While some interesting traits (e.g., occurrence of phages) could be further explored, we will not expend this analysis here. ## Final remarksApart from the protein clustering step, we completed these two pangenomic analyses within hours, including selecting core and peripheral genes, and creating figures in this popst. In fact writing this blog post took more than both steps combined. Our analyses can fully reproduced by downloading related anvi'o files from [here](http://dx.doi.org/10.6084/m9.figshare.1603512). With anvi'o version 1.2.1, it will take a few seconds to inter the interface and interact with these pangenomes and their associated metadata. If you like contests, you can add more metadata and share your edits in the [anvi'o forum](https://groups.google.com/forum/#!forum/anvio/home). Also, If you test anvi'o on your pangenomic data, feel free to ask questions or share your experience, preliminary results and ideas in the [anvi'o forum](https://groups.google.com/forum/#!forum/anvio/home)!

## Quick workflow for experimented users

After you have created a gene/protein cluster table (table.txt) and a metadata table (samples-information.txt), command lines to invoke are as follow:
