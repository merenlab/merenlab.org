...
NMDS = data.frame(MDS1 = mds$points[,1],
                  MDS2 = mds$points[,2],
                  method=with(metadata, get("method")),
...
DRAW_TOGETHER <- function(){
    if (length(only_two_groups) > 0){
		...
		p <- p + geom_point(aes(color = group, shape=method), size=4)
		p <- p + geom_path(data=df_ell, aes(x=MDS1, y=MDS2, colour=group), size=0.5, linetype=1)
		p <- p + geom_polygon(data=df_ell, aes(x=MDS1, y=MDS2, fill=group), alpha=0.1)
		p <- p + annotate("text",x=NMDS.mean$MDS1,y=NMDS.mean$MDS2,label=NMDS.mean$group, size=5)
		p <- p + annotate("text", x=NMDS$MDS1, y=NMDS$MDS2, label=NMDS$first, size=3)
		p <- p + ggtitle(options$title)
		p <- p + theme_bw()
	}
